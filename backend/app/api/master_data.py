...
def get_cities(db: Session = Depends(get_db)):
    return db.query(City).filter(City.is_active == True).order_by(City.sort_order, City.name).all()
...
def add_city(name: str, province: str = None, db: Session = Depends(get_db),
    ...
    db.add(city); db.commit(); return city
...
def delete_city(city_id: str, db: Session = Depends(get_db),
    ...
    city.is_active = False; db.commit(); return {"message": "Deleted"}
...
def get_insurers(db: Session = Depends(get_db)):
    ...
    return [{"id": i.id, "name": i.name, "branches": [{"id": b.id, "name": b.name, "city": b.city} for b in i.branches if b.is_active]} for i in insurers]

@router.post("/insurers", status_code=201)
def add_insurer(name: str, db: Session = Depends(get_db),
                current_user=Depends(require_role(UserRole.super_admin))):
    ins = Insurer(name=name); db.add(ins); db.commit(); return ins

@router.post("/insurers/{insurer_id}/branches", status_code=201)
def add_branch(insurer_id: str, name: str, city: str = None, db: Session = Depends(get_db),
               current_user=Depends(require_role(UserRole.super_admin))):
    branch = InsurerBranch(insurer_id=insurer_id, name=name, city=city)
    db.add(branch); db.commit(); return branch
...
def delete_insurer(insurer_id: str, db: Session = Depends(get_db),
    ...
    ins.is_active = False; db.commit(); return {"message": "Deleted"}
...
def delete_branch(branch_id: str, db: Session = Depends(get_db),
    ...
    branch.is_active = False; db.commit(); return {"message": "Deleted"}
...
def get_manufacturers(db: Session = Depends(get_db)):
    ...
    return [{"id": m.id, "name": m.name, "makes": [{"id": mk.id, "name": mk.name, "variants": [{"id": v.id, "name": v.name, "engine_cc": v.engine_cc, "body_type": v.body_type} for v in mk.variants if v.is_active]} for mk in m.makes if mk.is_active]} for m in mfrs]

@router.post("/manufacturers", status_code=201)
def add_manufacturer(name: str, db: Session = Depends(get_db),
                     current_user=Depends(require_role(UserRole.super_admin))):
    m = VehicleManufacturer(name=name); db.add(m); db.commit(); return m

@router.post("/manufacturers/{mfr_id}/makes", status_code=201)
def add_make(mfr_id: str, name: str, db: Session = Depends(get_db),
             current_user=Depends(require_role(UserRole.super_admin))):
    mk = VehicleMake(manufacturer_id=mfr_id, name=name); db.add(mk); db.commit(); return mk
...
def add_variant(make_id: str, name: str, engine_cc: str = None, body_type: str = None,
                db: Session = Depends(get_db), current_user=Depends(require_role(UserRole.super_admin))):
...
def delete_manufacturer(mfr_id: str, db: Session = Depends(get_db),
    ...
    m.is_active = False; db.commit(); return {"message": "Deleted"}
...
def delete_make(make_id: str, db: Session = Depends(get_db),
    ...
    mk.is_active = False; db.commit(); return {"message": "Deleted"}
